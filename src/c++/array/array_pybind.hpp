...
Pure C++ array payload exposed to Python through a NumPy bridge.
        ...
        .def(py::init<>())
        .def(py::init([](py::object object) {
            return arraybridge::arrayFromPyObject(object);
        }))

        .def("__getitem__", [](const Array& self, py::object key) {
            py::object value = arraybridge::toPyObject(self).attr("__getitem__")(key);
            return arraybridge::arrayFromPyObject(value);
        })
        .def("__setitem__", [](Array& self, py::object key, py::object value) {
            arraybridge::toPyObject(self).attr("__setitem__")(key, value);
        })
        ...
        .def("getItemAtIndex", [](Array& self, const size_t& flatIndex) -> py::object {
            ...
            if (self.hasDataOfType<int64_t>()) { return py::cast(self.getItemAtIndex<int64_t>(flatIndex)); }
            if (self.hasDataOfType<uint8_t>()) { return py::cast(self.getItemAtIndex<uint8_t>(flatIndex)); }
            if (self.hasDataOfType<uint16_t>()) { return py::cast(self.getItemAtIndex<uint16_t>(flatIndex)); }
            if (self.hasDataOfType<uint32_t>()) { return py::cast(self.getItemAtIndex<uint32_t>(flatIndex)); }
            if (self.hasDataOfType<uint64_t>()) { return py::cast(self.getItemAtIndex<uint64_t>(flatIndex)); }
            if (self.hasDataOfType<float>()) { return py::cast(self.getItemAtIndex<float>(flatIndex)); }
            if (self.hasDataOfType<double>()) { return py::cast(self.getItemAtIndex<double>(flatIndex)); }
        ...
        .def("__repr__", [](const Array& self) {
            ...
            repr << "Array(" << py::str(arraybridge::toPyObject(self).attr("__repr__")()).cast<std::string>() << ")";
            return repr.str();
        });
